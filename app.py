...
from utils.tts_engine import (
    generate_tts_audio, audio_player, audio_player_dual, audio_player_pausable, audio_player_autoplay_silent, audio_player_autoplay_playlist,
...
def home_page():
    ...
    def _collect_home_ai_task():
        task_id = str(st.session_state.get("home_ai_task_id", "") or "")
        if not task_id:
            return
        if time.time() < float(st.session_state.get("home_ai_earliest_show_at", 0.0) or 0.0):
            return
        with _HOME_AI_TASK_LOCK:
    ...
    with right_col:
        ...
        if google_needs_refresh or target_changed or mode_changed:
            try:
                if current_input:
                    ...
                    st.session_state.home_ai_earliest_show_at = time.time() + 0.9
        ...
        if history_source_text and selected_target.get("label"):
            ...
            if (
                ...
                and str(st.session_state.get("home_translation_target_used", "") or "") == selected_target["key"]
                and (
                    selected_target["key"] != "japanese"
                    or str(st.session_state.get("home_translation_japanese_mode_used", "normal") or "") == japanese_mode
                )
            ):
                upsert_translation_history_entry(
    ...
    with st.expander("Translation History", expanded=False):
        ...
        if not history_entries:
            ...
            source_options = ["All"] + sorted(
                [x for x in history_df.get("translation_source", pd.Series([], dtype=str)).dropna().astype(str).unique().tolist() if x.strip()]
            )
            lang_options = ["All"] + sorted(
                [x for x in history_df.get("target_language", pd.Series([], dtype=str)).dropna().astype(str).unique().tolist() if x.strip()]
            )
            f1, f2 = st.columns(2)
            with f1:
            ...
            with f2:
                selected_language = st.selectbox(
                    ...
                    lang_options,
                    index=0,
                    key="history_filter_target_language",
                )

            filtered_df = history_df.copy()
            if selected_source != "All":
                filtered_df = filtered_df[filtered_df["translation_source"] == selected_source]
            if selected_language != "All":
                filtered_df = filtered_df[filtered_df["target_language"] == selected_language]
